# 다른 PC에서 작업 재개하기 (사지방 체크리스트)

> 사지방처럼 **매번 초기화되는 공용 PC**에서 이 프로젝트를 이어서 작업할 때의 절차.
> 위에서부터 순서대로 따라가면 된다.
...
백업 JSON 은 **공용 PC가 아니라 본인 저장소**(USB, 개인 클라우드, 메일 등)에 둔다.
...
> Node 가 없고 설치도 못 하는 PC라면: 집에서 미리 `npm run build` 한 `dist/` 폴더를
> USB 로 들고 와서 `dist/index.html` 을 더블클릭해도 앱은 돈다. 코드 수정은 못 한다.
...
3. 초기화된 PC라면 기존 데이터가 없으므로 바로 들어온다.
   ...
   - **병합** — 집과 사지방 양쪽에서 각각 기록했을 때. 겹치는 건 건너뛰고 새 것만 추가한다.
