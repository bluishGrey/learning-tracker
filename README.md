...
된다 (Week 0 … Week 10). 그러면 분모가 곧 전체 커리큘럼이 된다. 두 변경이 한 쌍인
...
- 선택은 `settings` 가 아니라 **별도 localStorage 키**에 둔다. 백업 JSON 에 섞이면
  집 PC의 취향이 사지방 PC로 따라가 덮어쓴다 — 기록 동기화와 상관없는 값이다.
...
커리큘럼 뼈대(Week 0 … Week 10)를 한 번에 세우는 것도 **과목 정보**가 맡는다 —
...
과목만 자동 생성하지 않는 이유는 오타 사고 때문이다. `CS50x` 를 `CS50X` 로 한 번
