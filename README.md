...
블록 상세의 **내보내기** 버튼이 그 블록의 모든 기록을 클립보드에 담는다.
claude.ai 같은 곳에 그대로 붙여넣는 용도라 마크다운이나 JSON 으로 감싸지 않는다.
...
- `svgCode` 가 있는 기록이 하나도 없으면 SVG 구획도 만들지 않는다.
- `navigator.clipboard` 는 보안 컨텍스트에서만 동작한다. 막히면 조용히 실패하지 않고
  **원문을 띄워 직접 복사**하게 한다 (`file://` 로 열었을 때 대비).
